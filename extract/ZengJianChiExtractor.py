...
class ZengJianChiExtractor(object):
    ...
    def __init__(self, config_file_path, ner_model_dir_path, ner_blacklist_file_path):
        ...
        for table_dict_field in self.config['table_dict']['fields']:
            ...
            # print(self.table_dict_field_pattern_dict[field_name])
    ...
    def extract_from_table_dict(self, table_dict):
        ...
        for i in range(col_length):
            ...
            for (field_name, table_dict_field_pattern) in self.table_dict_field_pattern_dict.items():
                # print(field_name)
                # print(table_dict_field_pattern)
    ...
    def extract_company_name(self, paragraph):
        ...
        for target in targets:
            # print("===> ", target.group())
            com_abbr = target.group("com_abbr")  # 公司简称
            com_name = target.group("com")  # 公司全称
            # print(com_abbr)
            # print(com_name)
    ...
    def extract_from_paragraph(self, paragraph):
        tag_res = self.ner_tagger.ner(paragraph, self.com_abbr_ner_dict)  # 已提取出上一个html文件的公司简称
        # print("----------------------------")
        # print(paragraph)
        # print(tag_res.get_tagged_seg_list())
        tagged_str = tag_res.get_tagged_str()
        # print(tagged_str)
        #抽取公司简称
        new_size = self.extract_company_name(tagged_str)
        if new_size > 0:  # 在第二段文本中提取到了公司全称和简称
            tag_res = self.ner_tagger.ner(paragraph, self.com_abbr_ner_dict)
            tagged_str = tag_res.get_tagged_str()
            # print(tagged_str)

        change_records = self.extract_change(tagged_str)  # 变动信息
        change_after_records = self.extract_change_after(tagged_str)  # 变动后持股数等
    ...
    def extract_change(self, paragraph):
        ...
        for target in targets:
            ...
            m_com = pat_com.findall(paragraph,0,end_pos)  # 所有机构名称、简称
            # print(m_com)
            shareholder = ""  # 简称
            ...
            if m_date != None and len(m_date)>0:
                change_date = m_date[-1]  # 增减持股份数量的前面最近的日期，一般是增减持时间
                # print(change_date)
            #查找变动价格
    ...
    def extract(self, html_file_path):
        # 1. 解析 Table Dict
        rs = []
        paragraphs = self.html_parser.parse_content(html_file_path)  # 得到每段话为元素的列表
        rs_paragraphs = self.extract_from_paragraphs(paragraphs)
        # print("------------------------------------------")
        # print(paragraphs)
        # for x in rs_paragraphs:
        #     print(x)
